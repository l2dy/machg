...
var highlightDiff = function(diff, element, callbacks)
{
	...
	for (var lineno = 0, lindex = 0; lineno < lines.length; lineno++)
	{
		...
		else if (firstChar == "@")
		{
			...
			'<span class="includehunk"><input type="checkbox" class="hunkselector" checked="yes" onclick="handleHunkStatusClick(event)" id="' + encodedFileNamePlusHunk + '">commit</input></span></div>';
		}
		else if (firstChar == " ")
		{
			line1 += ++hunk_start_line_1 + "\n";
